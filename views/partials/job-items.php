<?php if (cwrm_getData('list_page') == 'false') { ?>
...
<?php foreach ($jobs as $job) { ?>
    <div class="cwrm-jobs-item cwrm-jobs-item-<?php esc_attr_e($job['id']); ?>">
        <h2 class="cwrm-list-page-title cwrm-list-page-title-<?php esc_attr_e($job['id']); ?>">
        ...
        <span> <span class="dashicons dashicons-clock"></span> <?php _e('Last Date', 'wp-recruit-manager'); ?></span> : <?php echo date('d M, Y', strtotime($job['last_date'])); ?><br />
        </div>
        <?php } ?>
        ...
        <?php if ($job['terms']) { ?>
            ...
            <span class="dashicons dashicons-paperclip"></span> <?php echo '<span>'.esc_html(cwrm_replaceHyphen($taxonomy)).'</span> : '.esc_html(implode(', ', $term)); ?><br />
            </div>
            <?php } ?>
            <?php } ?>
        <?php } ?>
